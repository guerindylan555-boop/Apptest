...
export const AutomationController: React.FC<AutomationControllerProps> = ({ className }) => {
  ...
  const [events, setEvents] = useState<Array<{time: string, message: string, type: 'info' | 'success' | 'error' | 'llm'}>>([]);
  ...
  const addEvent = (message: string, type: 'info' | 'success' | 'error' | 'llm' = 'info') => {
