...
class ScoringEngine {
    ...
    async scoreNode(dump, node) {
        ...
        const totalScore = hashScore * this.config.hashMatchWeight +
    ...
    scoreHashMatch(dump, signature) {
        ...
        const resourceIdOverlap = this.calculateArrayOverlap(dump.resourceIds.sort(), signature.resourceIds.sort());
        return Math.max(activityMatch, resourceIdOverlap * 25);
    }
    /**
     * Score selector match based on resource IDs and texts present in dump
     */
    scoreSelectorMatch(dump, node) {
        let totalScore = 0;
        ...
        for (const selector of node.selectors) {
            ...
            switch (selector.type) {
                case 'resource-id':
                    ...
                    break;
                case 'text':
                    if (dump.visibleTexts.some(text => text.toLowerCase().includes(selector.value.toLowerCase()))) {
     ...
     */
    scoreLayoutSimilarity(dump, signature) {
        ...
        const distance = this.levenshteinDistance(dump.layoutFingerprint, signature.layoutFingerprint);
    ...
    generateSignatureHash(dump) {
        ...
        return crypto_1.default.createHash('sha256').update(hashInput).digest('hex').substring(0, 16);
    }
    /**
     ...
     */
    createDetectionResult(candidates, dumpPath) {
